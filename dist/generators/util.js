...
function hydrateRequest(context, request) {
    ...
    hydrated.className = helpers_2.ensureSuffix(change_case_1.pascalCase(request.name), change_case_1.upperCaseFirst(suffix));
    hydrated.fileName = helpers_2.removeSuffix(change_case_1.paramCase(request.name), "-" + change_case_1.paramCase(suffix));
    hydrated.dirToRead = path_1.join(helpers_1.getStringPropertyValue(Constants.ENV_VAR_IONIC_ANGULAR_TEMPLATE_DIR), request.type);
...
function hydrateTabRequest(context, request) {
    ...
    for (var i = 0; i < request.tabs.length; i++) {
        var tabVar = change_case_1.camelCase(request.tabs[i].name) + "Root";
        ...
        if (i === request.tabs.length - 1) {
            hydrated.tabContent += "    <ion-tab [root]=\"" + tabVar + "\" tabTitle=\"" + change_case_1.sentenceCase(request.tabs[i].name) + "\" tabIcon=\"information-circle\"></ion-tab>";
        }
        else {
            hydrated.tabContent += "    <ion-tab [root]=\"" + tabVar + "\" tabTitle=\"" + change_case_1.sentenceCase(request.tabs[i].name) + "\" tabIcon=\"information-circle\"></ion-tab>\n";
        }
...
function readTemplates(pathToRead) {
    ...
    var promises = absolutePaths.map(function (absolutePath) {
        var promise = helpers_1.readFileAsync(absolutePath);
...
function applyTemplates(request, templates) {
    ...
    templates.forEach(function (fileContent, filePath) {
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.CLASSNAME_VARIABLE, request.className);
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.FILENAME_VARIABLE, request.fileName);
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.SUPPLIEDNAME_VARIABLE, request.name);
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.TAB_CONTENT_VARIABLE, request.tabContent);
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.TAB_VARIABLES_VARIABLE, request.tabVariables);
...
function createDirAndWriteFile(filePath, fileContent) {
    ...
    return helpers_1.mkDirpAsync(directory).then(function () {
        return helpers_1.writeFileAsync(filePath, fileContent);
    });
}
function getNgModules(context, types) {
    var ngModuleSuffix = helpers_1.getStringPropertyValue(Constants.ENV_NG_MODULE_FILE_NAME_SUFFIX);
...
function nonPageFileManipulation(context, name, ngModulePath, type) {
    ...
    var fileContent;
    return helpers_1.readFileAsync(ngModulePath).then(function (content) {
        fileContent = content;
        return generateTemplates(context, hydratedRequest);
    }).then(function () {
        fileContent = typescript_utils_1.insertNamedImportIfNeeded(ngModulePath, fileContent, hydratedRequest.className, path_1.relative(path_1.dirname(ngModulePath), hydratedRequest.dirToWrite));
        fileContent = typescript_utils_1.appendNgModuleDeclaration(ngModulePath, fileContent, hydratedRequest.className);
        return helpers_1.writeFileAsync(ngModulePath, fileContent);
    });
}
...
function tabsModuleManipulation(tabs, hydratedRequest, tabHydratedRequests) {
    ...
    var tabsNgModulePath = hydratedRequest.dirToWrite + "/" + hydratedRequest.fileName + ".module.ts";
    return helpers_1.readFileAsync(tabsNgModulePath).then(function (content) {
        var fileContent = content;
        fileContent = typescript_utils_1.insertNamedImportIfNeeded(tabsNgModulePath, fileContent, tabHydratedRequests[0].className, path_1.relative(path_1.dirname(tabsNgModulePath), ngModulePath.replace('.module.ts', '')));
        fileContent = typescript_utils_1.appendNgModuleDeclaration(tabsNgModulePath, fileContent, tabHydratedRequests[0].className);
        return helpers_1.writeFileAsync(tabsNgModulePath, fileContent);
    });
}
